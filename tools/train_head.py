...
training and inference can never diverge) and fits the logistic head with
``cleancode.semantics.training.fit_logistic_regression`` — full-batch
gradient descent from a zero start, so the output is reproducible without a
seed. Needs only numpy:
...
from cleancode.semantics.training import fit_logistic_regression  # noqa: E402
...
def main() -> None:
    ...
    scores = 1.0 / (1.0 + np.exp(-(features @ weights + bias)))
    ...
    head = {
        ...
        "training": {"samples": len(labels), "accuracy": round(accuracy, 4)},
    }
    OUTPUT.write_text(json.dumps(head) + "\n", encoding="utf-8")
    print(f"training accuracy {accuracy:.3f}; wrote {OUTPUT}")
